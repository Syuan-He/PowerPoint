...
namespace PowerPoint
{
    public class WindowsFormsGraphicsAdaptor : IGraphics
    {
        ...
        {
            _graphics.DrawLine(Pens.Black, (int)(x1 * _ratio), (int)(y1 * _ratio), (int)(x2 * _ratio), (int)(y2 * _ratio));
        }

        //畫矩形
        public void DrawRectangle(int x1, int y1, int width, int height)
        {
            _graphics.DrawRectangle(Pens.Black, x1 * _ratio, y1 * _ratio, width * _ratio, height * _ratio);
        }

        //畫圓圈
        public void DrawEllipse(int x1, int y1, int width, int height)
        {
            _graphics.DrawEllipse(Pens.Black, x1 * _ratio, y1 * _ratio, width * _ratio, height * _ratio);
        }

        // 繪製選取外框
        public void DrawSelectFrame(int x1, int y1, int x2, int y2)
        {
            float ratioX1 = x1 * _ratio;
            float ratioY1 = y1 * _ratio;
            float ratioX2 = x2 * _ratio;
            float ratioY2 = y2 * _ratio;
            _graphics.DrawRectangle(Pens.Black, Math.Min(ratioX1, ratioX2), Math.Min(ratioY1, ratioY2), Math.Abs(ratioX2 - ratioX1), Math.Abs(ratioY2 - ratioY1));
